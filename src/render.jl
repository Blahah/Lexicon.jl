      ...
      wrap(io, "li") do
        wrap(io, "ul") do
          for (s, obj) in index[k]
            push!(ents, modulename(documentation), obj,
                  entries(documentation)[obj])
            wrap(io, "li") do
...
function writemime{category}(io::IO, mime::MIME"text/html", modname, obj, ent::Entry{category})
  wrap(io, "div", "class='entry'") do
    ...
    wrap(io,
         "div", "id='$(objname)' class='entry-name category-$(category)'") do
...
function save_sectioned(file::String, modulename::Module; mathjax = false)
  mime = MIME("text/$(strip(last(splitext(file)), '.'))")
  save_sectioned(file, mime, documentation(modulename); mathjax = mathjax)
...
function save_sectioned(file::String,
  mime::MIME"text/html",
  ...
  ent_dict = entries(documentation)
  ents = Entries()
  for (k, e) in ent_dict
    push!(ents, modulename(documentation), k, e)
  end
  ...
  if (length(ents) > 0)
    ...
    all_sections = sections(ents)
    wrap(io, "ul", "class='sections'") do
      for (section, section_ents) in all_sections
        wrap(io, "li") do
          print(io,
                "<a href='#module-$(lowercase(section))'><h3>$section</h3></a>")
        end
      end
    end
    for (section, section_ents) in all_sections
      writemime(io, mime, section_ents, section)
    end
...
function writemime(io::IO,
                    ...
                    mathjax = false)
  if (length(section) > 0)
    println(io, "<h2 id='module-$(lowercase(section))'>$section</h2> <small><a href='#module-reference'>(back to top)</a></small>")
  end

  index = Dict{Symbol, Any}()
  for (modname, obj, entry) in ents.entries
    addentry!(index, obj, entry)
  end
...
function sections(ents::Entries)
  sects = Dict{String, Entries}()
  for (modname, obj, ent) in ents.entries
    md = metadata(ent)
    if (haskey(md, :section))
      sect = md[:section]
    else
      sect = ""
    end
    if (!haskey(sects, sect))
      sects[sect] = Entries()
    end
    push!(sects[sect], modname, obj, ent)
  end
  return sects
end
